...
def DynamicShortest(serviceTimes, arrivalTimes, maxAngents):
    dynHistory = []
    iteration = 0
    ...
    class ArrivalEvent:
        ...
        def Execute(self):
            ...
            if (len(DEVS.customerQueues) == 4 and idleCount() == 0):
                DEVS.servers.append(-1)
                DEVS.customerQueues.append([])
            ...
            # server is Busy
                ...
                shortestInd = shortestQueue()
                DEVS.customerQueues[shortestInd].append(customer.id)

                print("customerQueueLength of %i" % shortestInd, "= %d" % len(DEVS.customerQueues[shortestInd]))
    ...
    class ServiceEvent:
        ...
        def Execute(self):
            ...
            if (len(DEVS.customerQueues[serverId]) > 0):
                qid = DEVS.customerQueues[serverId].pop(0)
                ...
                if (len(DEVS.customerQueues) == 5 and idleCount() >= 2):
                    DEVS.customerQueues.pop(serverId)
    ...
    def printState():
        print("servers: %a;" %DEVS.servers, "queues: %a" %DEVS.customerQueues)


    def shortestQueue():
        bestInd = 0
        bestLen = len(DEVS.customerQueues[bestInd])

        for q in enumerate(DEVS.customerQueues):
            if len(q[1]) < bestLen:
                bestLen = len(q[1])
                bestInd = q[0]
    ...
    def DrawServersDynamic(dynHistory):
        plt.title("dynamic shrotest")
        plt.plot(dynHistory)
        plt.show()


    DrawSampleHistogram([x.waitingTimeInQueue for x in DEVS.stats], "dynamic shortest")
